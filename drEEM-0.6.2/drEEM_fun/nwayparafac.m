...
%   [Factors,it,err,corcondia]=<strong>nwayparafac</strong>(X,Fac,Options,const,OldLoad,FixMode,Weights)
%
% <a href="matlab: doc nwayparafac">help for nwayparafac</a> <- click on the link
...
% $ Version 2.17 $ Apr 2019 $ Remove temp.mat feature
NumbIteraInitia=20;
%accel_pattern='none';
...
if nargin<3
    OptionsDefault=[1e-06;0;0;1;10;2500];
else
...
if ~exist('OldLoad','var')
    OldLoad=0;
elseif isempty(OldLoad)
    OldLoad=0;
end
...
if size(Weights,1)==size(X,1)&&numel(Weights)/size(X,1)==size(X,2)
    Weights = reshape(Weights,size(Weights,1),numel(Weights)/size(X,1));
    if showfit~=-1
...
if MissMeth
    ...
    elseif ~(any(const==4)||any(const==5))
        SSX=sum(sum(X.^2)); % To be used for evaluating the %varnan explained
    else
        SSX=sum(abs(X(:)));
    end
end
...
if ~iscell(const)
    if any(const==3)||any(const==1)
        if DoWeight==1
            error(sprintf([' Weighting is not possible together with unimodality and orthogonality.\n'...
                ' It can be done using majorization, but has not been implemented here\n'...
...
elseif Init==10
    ...
    Opt(2) = 0;
    ERR=[];
    [Factors,~,err] = parafac(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
    ERR = [ERR;err];
    Opt(2) = 1;
    [F,~,Err] = parafac(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
    ERR=[ERR;Err];
    if Err<err
        Factors=F;
        err=Err;
...
if nargout>3
    ...
    any(isnan(ff{2}(:)));
    any(isnan(ff{3}(:)));
    % Inserted by Urban to handle error
    try
...
if showfit~=-1
    if ((f<crit) && (norm(connew-conold)/norm(conold)<MissConvCrit)) && it < maxit
