function [summary,M]  =  openfluormatches(filename)
%
% <strong>Syntax</strong>
%   <strong>openfluormatches</strong>(filename)
%
% <a href = "matlab: doc openfluormatches">help for openfluormatches</a> <- click on the link

% Plot match results from OpenFluor.
%
% USEAGE:
%     openfluormatches(filename)
%
% INPUTS:
...
%
% openfluormatches: Copyright (C) 2020 Urban J. Wuensch
%
...
try
    usetl=true;
    tl = tiledlayout(rc(nplots,1),rc(nplots,2),'padding','compact','TileSpacing','compact');
catch
    usetl=false;
end
...
for n = 1:numel(M.Creference)
    if usetl
        ...
        ax(n) = subplot(rc(nplots,1),rc(nplots,2),n);
    end
    hold(ax(n),'on')
    ...
    ylim(ax(n),[-0.05 1.05])
    set(ax(n),'YTickLabel','')
    xlabel(ax(n),'Wavelength (nm)')
    ylabel(ax(n),'Loadings')
