function [FMax,B,C,varargout]=modelexport(data,f,excelname,varargin)
%
% <strong>Syntax</strong>
%   [FMax,B,C,varargout]=<strong>modelexport</strong>(data,f,excelname,varargin)
%
% <a href="matlab: doc modelexport">help for modelexport</a> <- click on the link
...
% $ Version 0.3.0 $ June 2014 $ Second Release
% $ Version 0.1.0 $ May 2013 $ First Release
%
% bug fix 2015/09/19- fixed exported data for excluded samples
%
% Copyright (C) 2013, 2014 KR Murphy,
        ...
        if or(isfield(data,'Smooth'),isfield(fullds,'Smooth'))
            if ~and(isfield(data,'Smooth'),isfield(fullds,'Smooth'))
                ...
                if ~strcmp(fullds.Smooth,data.Smooth)
                    warning('Incompatible data about scatter removal in data.Smooth and fullds.Smooth. Press any key to continue or CTRL+C to cancel.')
...
disp('Exporting Info....')
writecell({'PARAFAC Model Report '},excelFileName,'Sheet',reportsheet,'Range','A1')
writecell({'Info'},excelFileName,'Sheet',reportsheet,'Range','A3')
writecell({'Toolbox'},excelFileName,'Sheet',reportsheet,'Range','B4')
writecell({toolbox},excelFileName,'Sheet',reportsheet,'Range','C4')
writecell({'Date'},excelFileName,'Sheet',reportsheet,'Range','B5')
writecell({datestr(now)},excelFileName,'Sheet',reportsheet,'Range','C5')
...
writecell({'Preprocessing'},excelFileName,'Sheet',reportsheet,'Range','A7')
writecell(PPHeaders,excelFileName,'Sheet',reportsheet,'Range','B8')

k=8;
for i=1:4
    if ~isempty(PPData{i})
        if ischar(PPData{i})
            writecell(cellstr(PPData{i}),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
%             xlswrite(excelFileName,cellstr(PPData{i}),reportsheet,['C' num2str(k)])
        elseif isnumeric(PPData{i})
            writematrix(PPData{i},excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
        end
    end
    k=k+1;
end
for i=1:length(ppOptNames)
    ...
    if isfield(data,ppvarin)
        if ischar(data.(ppvarin))
            writecell(cellstr(data.(ppvarin)),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
        else
            if strcmp(ppvarin,'Smooth')
                if numel(data.(ppvarin))==1
                smoothchar=[num2str(data.Smooth.ray1*data.Smooth.cutout(1)),',',...
                    num2str(data.Smooth.ram1*data.Smooth.cutout(2)),',',...
                    num2str(data.Smooth.ray2*data.Smooth.cutout(3)),',',...
                    num2str(data.Smooth.ram2*data.Smooth.cutout(4)),',',...
                    num2str(data.Smooth.interpolate),',',...
                    num2str(data.Smooth.d2zero),',','3382'];
                else 
                    smoothchar=[''];
                    for sf=1:numel(data.(ppvarin))
                        if ischar(data.Smooth(sf).samples)
                            samplesspec=data.Smooth(sf).samples;
                        else
                            samplesspec=num2str(data.Smooth(sf).samples);
                        end
                        smoothchar=[smoothchar,[' Samples:',samplesspec,'   Settings: ',num2str(data.Smooth(sf).ray1*data.Smooth(sf).cutout(1)),',',...
                            num2str(data.Smooth(sf).ram1*data.Smooth(sf).cutout(2)),',',...
                            num2str(data.Smooth(sf).ray2*data.Smooth(sf).cutout(3)),',',...
                            num2str(data.Smooth(sf).ram2*data.Smooth(sf).cutout(4)),',',...
                            num2str(data.Smooth(sf).interpolate),',',...
                            num2str(data.Smooth(sf).d2zero),',','3382']]; %#ok<AGROW>
                    end
                end
                writecell(cellstr(smoothchar),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
            else
                writecell(data.(ppvarin),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
            end
        end
    end
    k=k+1;
...
writecell({'PARAFAC model'},excelFileName,'Sheet',reportsheet,'Range',['A' num2str(k)])
writecell(MODHeaders,excelFileName,'Sheet',reportsheet,'Range',['B' num2str(k+1)])
writematrix([nComp;nEx;nEm],excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k+1)])
k=k+3;
...
for i=1:length(MOptNames)
    ...
    if isfield(data,Mvarin)
        writecell(cellstr(Mvarin),excelFileName,'Sheet',reportsheet,'Range',['B' num2str(k)])
        if ischar(data.(Mvarin))
            writecell(cellstr(data.(Mvarin)),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
        else
            writematrix(data.(Mvarin),excelFileName,'Sheet',reportsheet,'Range',['C' num2str(k)])
        end
        k=k+1;
    end
end
...
for i=1:size(valfields,1)
    ...
    if isfield(data,outname)
        if ~valheader
            writecell({'Validation'},excelFileName,'Sheet',reportsheet,'Range',['A' int2str(k)])
            valheader=true;
        end
        k=k+kplus+1;
        kplus=0;
...
%         xlswrite(excelFileName,cellstr(outname),reportsheet,namecell)
        writecell(cellstr(outname),excelFileName,'Sheet',reportsheet,'Range',namecell)
        datcell=['C' int2str(k)];
%         xlswrite(excelFileName,outdat,reportsheet,datcell)
        if iscell(outdat)
            writecell(outdat,excelFileName,'Sheet',reportsheet,'Range',datcell)
        elseif isnumeric(outdat)
            writematrix(outdat,excelFileName,'Sheet',reportsheet,'Range',datcell)
        end
...
writecell({'Spectra'},excelFileName,'Sheet',reportsheet,'Range',['A' int2str(k)])
writecell(['mode' 'nm' CompColHead],excelFileName,'Sheet',reportsheet,'Range',['B' int2str(k+1)])
writecell(exemhead,excelFileName,'Sheet',reportsheet,'Range',['B' int2str(k+2)])
writematrix(report,excelFileName,'Sheet',reportsheet,'Range',['C' int2str(k+2)])

%Export Loadings
disp('Exporting Loadings....')
writecell(loadheads,excelFileName,'Sheet',loadsheet,'Range','A1')
writematrix([indices FMax],excelFileName,'Sheet',loadsheet,'Range',[AtoZ(1) '2'])
writematrix([data.Ex C],excelFileName,'Sheet',loadsheet,'Range',[AtoZ(1+nComp+sepcol) '2'])
writematrix([data.Em B],excelFileName,'Sheet',loadsheet,'Range',[AtoZ(1+2*(nComp+sepcol)) '2'])

%Export Fmax for full dataset
if projectmodel
    disp('Exporting projected Fmax ....')
    writecell({'Fmax for full dataset including samples excluded from model building'},excelFileName,'Sheet',optsheet,'Range','A1')
    writecell({'i'},excelFileName,'Sheet',optsheet,'Range','A2')
    writecell(FmaxColHead,excelFileName,'Sheet',optsheet,'Range','B2')
    writematrix([(1:Db(1))' FMaxFull],excelFileName,'Sheet',optsheet,'Range','A3')
end
...
if nargin>4
    ...
    for i=1:size(metaflds,2)
        writecell(metaflds,excelFileName,'Sheet',metasheet,'Range','B1')
        if isnumeric(MD{i})
            writematrix(MD{i},excelFileName,'Sheet',metasheet,'Range',[AtoZ(i+1) '2'])
        elseif ischar(MD{i})
            writecell(MD{i},excelFileName,'Sheet',metasheet,'Range',[AtoZ(i+1) '2'])
        end
    end
    writematrix((1:nSample)',excelFileName,'Sheet',metasheet,'Range','A2')
end
